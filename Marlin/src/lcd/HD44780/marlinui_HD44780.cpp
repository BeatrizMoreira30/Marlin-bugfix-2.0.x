...
void MarlinUI::set_custom_characters(const HD44780CharSet screen_charset/*=CHARSET_INFO*/) {
  ...
  const static PROGMEM byte thermometer[2][8] = {
    {B00001,
     B00111,
     B01111,
     B01111,
  ...
  const static PROGMEM byte clock[8] = {
    ...
    { // Info Screen uses 5 special characters
      ...
      for (uint8_t i = 2; i--;)
        createChar_P(LCD_STR_THERMOMETER[i], thermometer[i]);
...
void MarlinUI::clear_lcd() { lcd.clear(); }
  ...
  static void logo_lines(FSTR_P const extra) {
    ...
    lcd_put_u8str(indent, 1, F("|Cooxupé|")); lcd_put_u8str(extra);
  ...
  void MarlinUI::show_bootscreen() {
    ...
    //
    if (LCD_EXTRA_SPACE >= (SHORT_BUILD_VERSION) + 1) {
      //
      // Show the Marlin logo, splash line1, and splash line 2
      //
      logo_lines(F("Projeto Terrus Café"));
      CENTER_OR_SCROLL("IFSULDEMINAS", 2000);
    }
    else {
      //
      ...
      CENTER_OR_SCROLL("PROJETO TERRUS CAFÉ", 1000);
      CENTER_OR_SCROLL("IFSULDEMINAS", 1000);
      CENTER_OR_SCROLL("EMBRAPII", 1000);
...
void MarlinUI::draw_status_screen() {
    ...
    // ========== Line 1 ==========
      ...
      //
        ...
        //
          ...
          _draw_heater_status(H_E1, LCD_STR_THERMOMETER[1], blink);
        #elif HAS_HEATED_BED
          lcd_moveto(8, 0);
      ...
      //
        ...
        lcd_moveto(1, 0);
        _draw_heater_status(H_E0, LCD_STR_THERMOMETER[1], blink);
  ...
  lcd_moveto(0,3);
  lcd_put_u8str(F("Projeto Terrus Cafe"));
}
